...
3. **Ability gating** — Toda mecánica avanzada pasa por `AliaAbilityController.HasAbility()`.
4. **Datos en ScriptableObjects** — Stats de ALIA en `AliaStatsSO` para iterar balance sin tocar código.

## Flujo de una partida

1. `GameManager` carga `SaveData` al iniciar.
2. ALIA spawnea en el último checkpoint (`SaveData.lastCheckpointId`).
...
|---------------------|------------|
| PlayerController existente | Renombrar o fusionar con `AliaController` |
| Save system propio | Migrar datos a `SaveData` |
| Input System nuevo | Adaptar `ReadInput()` en `AliaController` |
